...
describe User do
  shared_examples_for :field_with_only_cyrillic_letters do |field, options|
    ...
    it { should allow_value("Иван Жак").for(field) }
    it { should allow_value("Иван жак").for(field) }
    it { should_not allow_value("Иван_жак").for(field).with_message(options[:error_message]) }
  ...
  describe '#first_name' do
    ...
    it_should_behave_like :field_with_only_cyrillic_letters, :first_name, :error_message => /должно состоять только из русских букв/
  end

  describe '#middle_name' do
    it { should_not validate_presence_of :middle_name }
    it_should_behave_like :field_with_squish_normalizator, :middle_name
    it_should_behave_like :field_with_only_cyrillic_letters, :middle_name, :error_message => /должно состоять только из русских букв/
  end

  describe '#last_name' do
    it { should validate_presence_of :last_name }
    it_should_behave_like :field_with_squish_normalizator, :last_name
    it_should_behave_like :field_with_only_cyrillic_letters, :last_name, :error_message => /должна состоять только из русских букв/
  end
